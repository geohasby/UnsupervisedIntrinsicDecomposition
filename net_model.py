...
def make_net(input_img, is_training):
    ...
    def inception_branch(bid, input_branch, branch):
        ...
        with tf.name_scope("branch"+name):
            ...
            if branch[0] > 0:
                conv1x1_pre = tf.layers.conv2d(
                    input_branch, branch[0], (1, 1), name="conv1x1_pre" + name + "_" + str(bid), padding="same")
                conv1x1_pre = tf.layers.batch_normalization(
                    conv1x1_pre, training=is_training, name="conv1x1_pre" + name + "_bn"+str(bid))
                conv1x1_pre = tf.nn.elu(
                    conv1x1_pre, name="conv1x1_pre" + name + "_elu_"+str(bid))
                input_branch = conv1x1_pre

            #   Then std convolution
            conv = tf.layers.conv2d(input_branch, branch[2], (
                branch[1], branch[1]), name="conv" + name + "_" + str(bid), padding="same")
            conv = tf.layers.batch_normalization(
                conv, training=is_training, name="conv" + name + "_bn"+str(bid))
    ...
    def inception_block(lid, input_layer):
        with tf.name_scope("inception_"+str(lid)):
            ...
            for branch in inception_branches:
                branches.append(inception_branch(
                    lid+"branch", input_layer, branch))
    ...
    def skip_encoder_level(lid, input_layer):
        ...
        if lid-1 > 0:
            ...
            layer = tf.layers.max_pooling2d(
                level_input, (3, 3), (2, 2), name="down_maxpool_"+str(lid))
